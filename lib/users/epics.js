...
export const addUser: Epic = (action$, { createUuid, getNow, getState }) =>
  action$.filter(action => action.type === 'ADD_USER').mergeMap(action => {
    ...
    // jak na to?
    // vracet validation error, ale nechat error na globalu, jak?
    // vyhodit error, a zachycenim premapovat, imho :-)
    ...
    if (!user.isAnarchist) {
      return Observable.of({
        type: 'ADD_USER_ERROR',
        id: user.id,
        validationErrors: { isAnarchist: { type: 'required', prop: 'agree' } },
      });
    }
