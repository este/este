...
export const add100RandomUsers: Epic = (action$, { createUuid, getNow }) =>
  action$
    .filter(action => action.type === 'ADD_100_RANDOM_USERS')
    .mergeMap(action => {
      if (action.type !== 'ADD_100_RANDOM_USERS') throw Error(); // refinement

      const actions = range(0, 100).map(i => {
        const id = createUuid();
        const now = getNow() + i;
        return {
