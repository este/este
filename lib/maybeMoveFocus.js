...
export const dataCaretEdgeAttr = 'data-maybe-move-focus-caret-edge';

export type CaretEdge = 'none' | 'start' | 'end' | 'both';
...
const getCaretEdge = (target): ?CaretEdge => {
  ...
  while (node && node.nodeType === 1) {
    const caretEdge =
      ...
      node.getAttribute(dataCaretEdgeAttr);
...
const maybeMoveFocusOnKey = (
  ...
  if (caretEdge != null && caretEdge !== 'both') {
    if (caretEdge === 'none') return;
    if (caretEdge === 'start' && direction !== 'left') return;
    if (caretEdge === 'end' && direction !== 'right') return;
  }
